...
- **A bounded local interaction is useful.** SlimSAM gives a user a light
  single-frame trial without uploading data. It is deliberately separate from
  the authoritative offline Cellpose-SAM and SAM2.1 evaluations.
...
- **ADR-0055 Pages-first.** FrothSeg's companion web is a static SPA on
  GitHub Pages with no server. Its optional SlimSAM interaction is client-side;
  authoritative foundation inference remains offline.
...
- **Version**: `@huggingface/transformers` v4.2.0.
...
- **Stage**: the optional browser `segment` stage. Input is a decoded RGB frame (a sample, an uploaded real froth image,
...
- **Contract it satisfies**: the bounded-live inference contract. The encoder runs once per image; the decoder is
  ...
  report progress. Its historical canonical diagnostic now LOSES to the classical floor by 0.037 rather than
  clearing it (mean mask AP 0.365 vs 0.402, wins 4 of the 12 scored cases, though still 0.407 vs 0.182 under
  the glare control); the floor stays ahead on heavy motion-blur and defocus and has taken back most other
  cases. Those floor figures were 0.262, 10 of 13 and 0.081 until the 2026-08-01 corrections, which
