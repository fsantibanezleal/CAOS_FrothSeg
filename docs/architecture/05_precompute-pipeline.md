...
`data-pipeline/fslab/pipeline.py` orchestrates the **named stages** (frozen names/signatures, rework bodies):
...
Run: `python -m fslab.pipeline [all|<case_id>] [--seed N]` (or `scripts/precompute.{sh,ps1}`). It writes
