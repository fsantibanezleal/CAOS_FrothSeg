...
def test_manifest_matches_artifacts_and_gate(tmp_path: Path):
    m = pipeline.precompute("poly-normal", output_root=tmp_path)
...
def test_masks_roundtrip_to_same_instance_count(tmp_path: Path):
    m = pipeline.precompute("fine-froth", output_root=tmp_path)
    doc = json.loads((tmp_path / m["artifacts"]["masks"]["path"]).read_text(encoding="utf-8"))
...
def test_benchmark_scores_present_for_every_floor_method(tmp_path: Path):
    m = pipeline.precompute("poly-normal", output_root=tmp_path)
