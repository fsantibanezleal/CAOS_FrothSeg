  ...
  return (
    <div className="page-body prose">
      <div className="page-head">
        ...
        <p className="lede">
          {es ? 'El stack real, las dos vías de cómputo, los dos contratos de datos y el pipeline offline que hornea el benchmark.' : 'The real stack, the two compute lanes, the two data contracts, and the offline pipeline that bakes the benchmark.'}
        </p>
      </div>
...
function Stack({ es }: { es: boolean }) {
  return (
    <>
      <p>{es ? 'Todo corre en el cliente; no hay backend. Las piezas, cada una la herramienta estándar para su tarea:' : 'Everything runs client-side; there is no backend. The pieces, each the standard tool for its job:'}</p>
...
function Lanes({ es }: { es: boolean }) {
  return (
    <>
      <p>
        {es
          ? 'Dos vías. La vía EN VIVO es el segmentador SAM en el navegador: el modelo se descarga una vez desde el hub de Hugging Face y el navegador lo cachea (Cache API); la inferencia corre en el cliente sobre tu cuadro con WebGPU. Si WebGPU no está, cae a WASM-SIMD, más lento pero funcional.'
          : 'Two lanes. The LIVE lane is the browser SAM segmenter: the model is fetched once from the Hugging Face hub and the browser caches it (Cache API); inference runs client-side on your frame with WebGPU. If WebGPU is absent it falls back to WASM-SIMD, slower but functional.'}
      </p>
      <p>
        {es
