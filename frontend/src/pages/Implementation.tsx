  ...
  return (
    <div className="prose">
      ...
      <Callout variant="honest" title={es ? 'Paridad es acuerdo, no calidad' : 'Parity is agreement, not quality'}>
        {es
          ? 'Los tres gemelos aceptados son parte del piso clásico y están por debajo de la barra de comparación: sus AP medias sobre la prueba intacta son 0,065, 0,220 y 0,198. Que coincidan con su versión offline significa que el navegador no miente sobre lo que ejecuta, no que sean buenos. El carril vivo es exploratorio por diseño y el líder medido no está en el navegador; una dependencia instalada tampoco prueba una implementación, porque el registro exige checkpoint o configuración, inferencia, evaluación retenida, artefactos y documentación.'
          : 'The three accepted twins are part of the classical floor and sit below the comparison bar: their mean untouched-test AP values are 0.065, 0.297 and 0.198, and only the middle one is anywhere near the learned lane. Their agreement with the offline version means the browser does not lie about what it runs, not that they are good. The live lane is exploratory by design and the measured leader is not in the browser; an installed dependency does not prove an implementation either, because the registry demands a checkpoint or configuration, inference, held-out evaluation, artifacts, and documentation.'}
        {/* C1 0.0652, C3 0.2975, C4 0.1977 test mean_ap: data/derived/method-benchmark.json. C3 moved with the
            2026-08-01 flooding-surface adoption (verification/phase1-adoption.json); the twin moved with it and
            its parity numbers improved to AP delta 0.0020 and mask IoU 0.9343 (verification/classical-live-parity.json). */}
      </Callout>
...
function Evidence({ es, refsLabel }: { es: boolean; refsLabel: string }) {
  return (
    <div className="prose">
      ...
      <p className="measure">
        {es
          ? 'La evidencia de cómputo es igual de obligatoria que la métrica. Cada método debe declarar carril de hardware, dispositivo, milisegundos medios de inferencia, memoria pico con la métrica usada para medirla, tamaño del artefacto de inferencia, si ese artefacto está versionado, y duración de entrenamiento; un método aprendido sin artefacto dimensionado y hasheado es un error de inventario, no una nota. Las cifras hacen visible el costo real de la escalera: el motor fundacional de segmentación celular llega a 8128,8 MiB de memoria pico y 486,5 ms por imagen, mientras el modelo de investigación entrenado aquí usa 36,5 MiB y 87,2 ms. El rastreador de video oficial declara 2090,2 MiB y 972,4 ms, pero esa cifra de milisegundos es un reloj de corrida completa dividido por el número de imágenes e incluye la carga del checkpoint: propaga desde prompts del primer cuadro y no tiene carril de imagen suelta que medir, y el banco ahora lo etiqueta en vez de ocultarlo. Esa diferencia es una de las razones por las que la comparación completa es offline.'
          : 'Compute evidence is as mandatory as the metric. Every method must declare its hardware lane, device, mean inference milliseconds, peak memory with the metric used to measure it, inference-artifact size, whether that artifact is versioned, and training duration; a learned method without a sized, hashed artifact is an inventory error, not a footnote. The numbers make the real cost of the ladder visible: the foundation cell-segmentation engine reaches 8128.8 MiB peak memory and 486.5 ms per image, while the research model trained here uses 36.5 MiB and 87.2 ms. The official video tracker reports 2090.2 MiB and 972.4 ms, but that millisecond figure is a whole-run wall-clock divided by the image count and includes checkpoint loading: it propagates from first-frame prompts and has no unprompted single-image lane to time, which the benchmark now labels rather than hides. That gap is one of the reasons the complete comparison is offline.'}
        {' '}<Cite id="stringer2021cellpose" paren />
        {/* peak_memory_mib and mean_inference_ms for L5 (8128.8 / 486.54), L7 (2090.2 /
            972.421875, timing_source "no-single-image-lane") and N1 (36.5 / 87.22):
      ...
      <Callout variant="honest" title={es ? 'El inventario está incompleto, y lo dice' : 'The inventory is incomplete, and says so'}>
        {es
          ? 'El reporte de release actual se declara incompleto con un único error bloqueante: no hay una fuente real de espuma, licenciada y aceptada, con muestras importadas y calibración física. Se mantiene como error que bloquea, no como nota al pie. Desde el 2026-08-01 ese error registra un estado del mundo cerrado, no una adquisición pendiente: dos pasadas independientes, una revisión de licencias candidato por candidato el 2026-07-28 y una pasada de investigación separada el 2026-07-31, concluyeron ambas que no hay disponible en repositorios públicos ningún dataset de espuma real, con licencia abierta y anotación por burbuja. La afirmación es que no hay ninguno disponible, no que no pueda existir, y la única ruta registrada es un conjunto anotado a propósito para este trabajo. Cerrar la pregunta no cambia nada de lo que se afirma: el error sigue bloqueando y ninguna cifra de aquí es exactitud de planta. Además, el liderazgo medido es específico del generador: el modelo de investigación lidera el benchmark sintético controlado con AP 0,5186 frente a 0,5099 del motor fundacional, pero la prueba de transferencia a dominio adyacente lo baja a 0,125 mientras el motor fundacional sube a 0,709; los seis modelos entrenados aquí se degradan sin excepción (media -0,243) y cinco de los siete métodos clásicos mejoran (media del nivel +0,070), con las dos excepciones nombradas en vez de promediadas: la cuenca de agua por inmersión sin marcadores cae de 0,0173 a 0,0, y C3 cae de 0,2196 a 0,1278 tras adoptarse su superficie de inundación el 2026-08-01. El documento no declara superioridad, y esa prueba adyacente favorece el dominio de preentrenamiento del motor fundacional, así que tampoco muestra que sea mejor en espuma: muestra que la ventaja del modelo propio no sobrevive el cambio de dominio. Una evaluación posterior, prerregistrada, sobre un split real fresco y nunca observado de 72 muestras situó al mismo ensamble publicado en AP 0,0446, materialmente por debajo del 0,1249 medido en el split anterior de 64 imágenes, mientras su cifra sintética se replicó dentro de 0,0041 en una reserva intacta: el desplome por cambio de dominio es más profundo de lo que sugería la primera medición, no menos.'
          : 'The current release report declares itself incomplete with a single blocking error: there is no accepted, licensed real froth source with imported samples and physical calibration. It is kept as a blocking error, not as a footnote. Since 2026-08-01 that error records a settled state of the world rather than a pending acquisition: two independent passes, a candidate-by-candidate licence review on 2026-07-28 and a separate research pass on 2026-07-31, both concluded that no openly licensed, real, per-bubble-annotated froth dataset is available in public repositories. The claim is that none is available, not that none can exist, and the only route on record is a set annotated on purpose for this work. Settling the question changes nothing that is claimed: the error stays blocking and no number here is plant accuracy. Beyond that, the measured lead is generator-specific: the research model leads the controlled synthetic benchmark with AP 0.5186 against 0.5099 for the foundation engine, but the adjacent-domain transfer test drops it to 0.125 while the foundation engine rises to 0.709; all six models trained here degrade without exception (mean -0.243) and five of the seven classical methods improve (tier mean +0.071), with the two exceptions named rather than averaged away: the marker-less immersion watershed falls from 0.0173 to 0.0, and C3 falls from 0.2975 to 0.2165 after its flooding surface and then its flooding depth were adopted on 2026-08-01. The document claims no superiority, and that adjacent test favours the foundation engine pretraining domain, so it does not show it is better on froth either: it shows the in-repository lead does not survive domain shift. A later pre-registered evaluation on a fresh, previously unobserved real split of 72 samples put the same published ensemble at AP 0.0446, materially below the 0.1249 measured on the earlier 64-image split, while its synthetic score replicated within 0.0041 on an untouched reserve slice: the domain-shift collapse is deeper than the first measurement suggested, not shallower.'}
        {/* complete false and the single blocking error: data/derived/release-report.json.
            0.5186 / 0.5099 / 0.125 / 0.709 / -0.243 / +0.071 and beyond_sota_claim false:
            data/derived/method-benchmark.json (current_bar) and data/derived/real-adjacent-benchmark.json.
            FIVE of seven, not six and not every, recomputed from those two artifacts on 2026-08-01:
            C2 watershed_immersion goes from test mean_ap 0.017265625 (method-benchmark.json,
            methods[].id == C2, .test) to mean_ap 0.0 (real-adjacent-benchmark.json, methods[].id == C2),
            a delta of -0.0173; C3 goes from 0.2975 to 0.21648438, a delta of -0.0810, which appeared
            when the C3 flooding surface was adopted (verification/phase1-adoption.json) and then
            softened from -0.0917 when its flooding depth was corrected the same day
            (verification/r2-c3-flooding-depth.json); the other five
            classical rows are positive and the +0.071 tier mean is exact over all seven. The earlier
            "six of seven, +0.088" wording predates the C3 adoption and is FALSE against the current
            artifacts; Introduction.tsx already carried the corrected count. Do not reintroduce it, and
            do not reintroduce the universal "every classical method improves" either.
            The settled data finding: the 2026-07-28 candidate-by-candidate licence search is CAOS_MANAGE
            wip/frothseg/real-data-source-search-2026-07-28.md, the independent 2026-07-31 research null
            is CAOS_MANAGE plans/frothseg/research-2026-07-31/SYNTHESIS.md section 7 item 13 and B-3, and
            the 2026-08-01 settlement of BL-012 is CAOS_MANAGE plans/frothseg/backlog.md plus status.md.
            The Kaggle candidate's absent licence is recorded here in manifests/source-registry.json
            under license_verification.
            The fresh-split numbers: verification/p2-domain-randomization.json, keys
            surface_replication.real_axis.fresh_split_mean_ap 0.04455555555555555 over fresh_split_n 72,
            surface_replication.real_axis.burned_split_mean_ap 0.12485937500000001 over 64, and
            results.A.synthetic_reserve_mean_ap 0.5226875 against the 0.51859375 reference, whose
            difference 0.00409375 is at verdict_detail.synthetic_clause.absolute_difference. That study's
            verdict is "null" and its scope line states the real axis is adjacent-domain cell-nucleus
            microscopy, not froth, so nothing in it supports a flotation accuracy claim. */}
      </Callout>
