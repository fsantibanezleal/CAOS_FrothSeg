...
function SplitsAndAggregation({ es, benchmark }: BenchmarkProps) {
  ...
  return (
    <div className="prose">
      ...
      <Refs ids={['kuhn1955hungarian', 'luiten2021hota', 'weaire1999foams', 'aurenhammer1987']} label="Refs" />
    </div>
  );
}
...
function LanesAndParity({ es }: { es: boolean }) {
  ...
  const parity: Array<{ id: string; name: { en: string; es: string }; delta: number; apAgree: number; boundaryAgree: number; ratio: number }> = [
    ...
    { id: 'C3', name: { en: 'Marker-controlled watershed', es: 'Watershed controlada por marcadores' }, delta: 0.01931, apAgree: 0.5226, boundaryAgree: 0.97856, ratio: 1.2420 },
    { id: 'C4', name: { en: 'Distance-transform watershed', es: 'Watershed por transformada de distancia' }, delta: 0.02556, apAgree: 0.6625, boundaryAgree: 0.95245, ratio: 0.8777 },
  ];
  return (
    <div className="prose">
...
function TransferDataset({ es, real }: { es: boolean; real: RealAdjacentDoc | null }) {
  ...
  return (
    <div className="prose">
      ...
      <Refs ids={['fu2019', 'aldrich2010', 'wang2003froth', 'weaire1999foams', 'aurenhammer1987']} label="Refs" />
    </div>
  );
}
...
function TransferResult({ es, benchmark, real }: { es: boolean; benchmark: MethodBenchmarkDoc | null; real: RealAdjacentDoc | null }) {
   ...
   *  C2 gradient immersion watershed drops from 0.017266 to 0.0 on the real lane.
  ...
  return (
    <div className="prose">
      ...
      <p className="measure">{es
        ? `Sobre ${real.n_samples} imágenes reales retenidas, con los ajustes de espuma aplicados sin cambios y ningún método reentrenado, el orden se reorganiza por completo. El método que lidera el banco de espuma con ${fmt(frothLeader?.froth)} cae a la posición ${frothLeaderRank} con ${fmt(frothLeader?.mean_ap)}, una caída de ${fmt(frothLeader ? frothLeader.froth - frothLeader.mean_ap : null)}. El nuevo líder es ${leader?.id} ${leader?.name} con ${fmt(leader?.mean_ap)}. El patrón no es ruido: cada modelo entrenado sobre las 192 muestras sintéticas se degrada, y ${classicalUp.length} de los ${classicalRows.length} métodos clásicos, que no tienen prior aprendido que sobreajustar, mejoran. La excepción es ${classicalDown.map((row) => `${row.id} ${row.name}`).join(', ')}, que cae de ${classicalDown.map((row) => row.froth.toFixed(3)).join(', ')} a ${classicalDown.map((row) => row.mean_ap.toFixed(3)).join(', ')} con un AP50 de ${classicalDown.map((row) => row.mean_ap50.toFixed(4)).join(', ')}: el exceso de segmentos que ya hundía su puntaje en espuma casi no deja emparejamientos aceptados sobre núcleos. El único método que nunca se entrenó en este repositorio es también el único método aprendido que mejora.`
        : `Over ${real.n_samples} real held-out images, with the froth settings applied unchanged and no method retrained, the order reorganises completely. The method leading the froth bench at ${fmt(frothLeader?.froth)} falls to position ${frothLeaderRank} at ${fmt(frothLeader?.mean_ap)}, a drop of ${fmt(frothLeader ? frothLeader.froth - frothLeader.mean_ap : null)}. The new leader is ${leader?.id} ${leader?.name} at ${fmt(leader?.mean_ap)}. The pattern is not noise: every model trained on the 192 synthetic samples degrades, and ${classicalUp.length} of the ${classicalRows.length} classical methods, which have no learned prior to overfit, improve. The exception is ${classicalDown.map((row) => `${row.id} ${row.name}`).join(', ')}, which falls from ${classicalDown.map((row) => row.froth.toFixed(3)).join(', ')} to ${classicalDown.map((row) => row.mean_ap.toFixed(3)).join(', ')} at an AP50 of ${classicalDown.map((row) => row.mean_ap50.toFixed(4)).join(', ')}: the surplus of segments that already sank its froth score leaves almost no accepted match on nuclei either. The one method never trained in this repository is also the only learned method that improves.`}{' '}<Cite id="stringer2021cellpose" paren /></p>
      ...
      <div className="two-col">
        ...
        <div>
          ...
          <ul>
            ...
            <li>{es ? 'Estabilidad entre ensambles del método líder: solo se evaluó una extracción.' : 'Ensemble-to-ensemble stability of the leading method: only one draw was evaluated.'}</li>
          </ul>
        </div>
      </div>
