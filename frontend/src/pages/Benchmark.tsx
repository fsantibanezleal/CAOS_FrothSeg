  ...
  return (
    <div className="fs-benchmark-chapter">
      ...
      <div className="fs-scope-columns">
        <article className="supported"><span>✓</span><div><strong>{es ? 'Sostenido por la evidencia' : 'Supported by the evidence'}</strong><ul><li>{es ? '15 implementaciones bajo el mismo test.' : '15 implementations under the same test.'}</li><li>{es ? 'Ranking, robustez, morfometría y costo medidos.' : 'Measured ranking, robustness, morphometry, and compute.'}</li><li>{es ? 'Cellpose-SAM lidera este banco con AP 0.510.' : 'Cellpose-SAM leads this benchmark at AP 0.510.'}</li><li>{es ? 'Cuatro métodos superan el umbral AP 0.30.' : 'Four methods exceed the AP 0.30 threshold.'}</li></ul></div></article>
      ...
      <p>{es ? 'LamellaStar fue implementado y evaluado, pero su resultado actual es inferior al líder y al umbral predeclarado. Se conserva para reproducibilidad y ablación; la aplicación no lo presenta como una mejora.' : 'LamellaStar was implemented and evaluated, but its current result is below both the leader and the predeclared threshold. It is retained for reproducibility and ablation; the application does not present it as an improvement.'}</p>
