  ...
  const regionTab = (
    <div className="prose">
      ...
      <Callout variant="honest" title={es ? 'Lo que este par no autoriza a decir' : 'What this pair does not license'}>
        <p>{es
          ? 'La cabeza del tier clásico está repartida y así se declara, aunque el reparto cambió el 2026-08-01 al corregir dos motores contra sus propias fuentes: sobre las 64 muestras retenidas C7 lidera en AP con 0.2326, seguido de C3 con 0.2196 y de C4 con 0.1977; C7 lidera además en F de frontera con 0.8837 y en error de conteo con 114.2, C4 sigue liderando en Wasserstein-1 de la distribución de tamaños con 2.590, y C3 tiene el mejor error relativo de d32 del tier con 0.1907, frente al 1.1555 de C4 y al 1.4371 de C7, que es el peor de los tres punteros aunque no el peor del tier (C5 marca 1.9224 y C1 5.0063, ninguno en disputa por la AP). Ninguno es el clásico más fuerte sin más: cuál conviene depende de si el consumidor es la máscara o la distribución de tamaños, y esa pregunta ahora separa a C7 de C3 con más nitidez que antes. Los tres siguen entre 0.28 y 0.32 de AP por debajo del líder aprendido, que marca 0.5186, así que el tier clásico no es una alternativa al carril entrenado sino su referencia. Ninguno de los dos cambios se adoptó por ganar un barrido: se adoptaron porque el motor no ejecutaba la fuente que el registro ya citaba, y ambos se confirmaron sobre una porción de reserva que ningún barrido había observado. La procedencia de C7 también está declarada con cuidado: es una línea base de valle o cresta del dominio cuya cita exacta permanece bajo compuerta en el registro, de modo que se acredita a la tradición de delineación de espuma y no se presenta como la reproducción de un artículo concreto. Y su nombre en el registro dice lo que el motor hace: watershed restringido de valle de lamela. Hasta el 2026-07-31 el registro lo llamaba detector de costura oscura, porque el motor no inundaba nada; la adopcion del 2026-08-01 resolvio ese desajuste implementando el metodo, asi que el nombre y la cita de Meyer 1994 volvieron al registro. Sigue sin usar OpenCV. C6 se conserva como control negativo, no como método propuesto.'
          : 'The head of the classical tier is split, and it is declared as split, though the split moved on 2026-08-01 when two engines were corrected against their own sources: over the 64 held-out samples C7 leads on AP with 0.2326, followed by C3 at 0.2196 and C4 at 0.1977; C7 also leads on boundary F with 0.8837 and on count error with 114.2, C4 still leads on the size-distribution Wasserstein-1 at 2.590, and C3 holds the tier\'s best d32 relative error at 0.1907 against C4\'s 1.1555 and C7\'s 1.4371, the worst of the three leaders though not of the tier (C5 records 1.9224 and C1 5.0063, neither in contention on AP). None is the strongest classical outright: which one is preferable depends on whether the consumer is the mask or the size distribution, and that question now separates C7 from C3 more sharply than before. All three remain between 0.28 and 0.32 AP below the learned leader at 0.5186, so the classical tier is not an alternative to the trained lane but its reference. Neither change was adopted for winning a sweep: both were adopted because the engine was not running the source the registry already cited, and both were confirmed on a reserve slice no sweep had observed. The provenance of C7 is also stated carefully: it is a domain valley or ridge baseline whose exact citation remains gated in the registry, so it is credited to the froth-delineation tradition and is not presented as the reproduction of one specific paper. And its registry name says what the engine does: a lamella-valley constrained watershed. Until 2026-07-31 the registry called it a dark-seam detector, because the engine flooded nothing; the 2026-08-01 adoption resolved that mismatch by implementing the method, so the name and the Meyer 1994 provenance were restored. It still uses no OpenCV. C6 is kept as a negative control, not as a proposed method.'}</p>
      </Callout>
      {/* C3 mean_ap 0.21957813 / boundary F 0.88173 / d32 0.19074, C4 mean_ap 0.19768750 / boundary F 0.83440 /
          BSD W1 2.590 / count abs err 139.8, C7 mean_ap 0.23262500 / boundary F 0.88369 / count abs err 114.2 /
          d32 1.43715: frontend/public/data/method-benchmark.json methods[2], methods[3] and methods[6], .test.
          Learned leader N1 mean_ap 0.51859375, same file, current_bar.leader. The two adopted defaults and
          their confirmation on the untouched reserve slice: verification/phase1-adoption.json.
