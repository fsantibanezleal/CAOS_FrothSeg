  ...
  const regionTab = (
    <div className="prose">
      ...
      <Callout variant="honest" title={es ? 'Lo que este par no autoriza a decir' : 'What this pair does not license'}>
        <p>{es
          ? 'La cabeza del tier clásico está repartida y así se declara: sobre las 64 muestras retenidas C4 lidera en AP con 0.1977 frente a 0.1673 de C7, mientras C7 lidera en F de frontera con 0.8628 y en error de conteo con 114.2, a 9.6 milisegundos por imagen, menos de la mitad de los 22.1 de C4. Ninguno es el clásico más fuerte sin más: cuál conviene depende de si el consumidor es la máscara o la distribución de tamaños. Ambos siguen entre 0.32 y 0.35 de AP por debajo del líder aprendido, así que el tier clásico no es una alternativa al carril entrenado sino su referencia. La procedencia de C7 también está declarada con cuidado: es una línea base de valle o cresta del dominio cuya cita exacta permanece bajo compuerta en el registro, de modo que se acredita a la tradición de delineación de espuma y no se presenta como la reproducción de un artículo concreto. Y su nombre en el registro dice lo que el motor hace: detector de costura oscura de valle de lamela, sin watershed y sin OpenCV, porque no usa ninguno de los dos. C6 se conserva como control negativo, no como método propuesto.'
          : 'The head of the classical tier is split, and it is declared as split: over the 64 held-out samples C4 leads on AP with 0.1977 against C7 at 0.1673, while C7 leads on boundary F with 0.8628 and on count error with 114.2, at 9.6 milliseconds per image, less than half C4 at 22.1. Neither is the strongest classical outright: which one is preferable depends on whether the consumer is the mask or the size distribution. Both remain between 0.32 and 0.35 AP below the learned leader, so the classical tier is not an alternative to the trained lane but its reference. The provenance of C7 is also stated carefully: it is a domain valley or ridge baseline whose exact citation remains gated in the registry, so it is credited to the froth-delineation tradition and is not presented as the reproduction of one specific paper. And its registry name says what the engine does: a lamella-valley dark-seam detector, with no watershed and no OpenCV, because it uses neither. C6 is kept as a negative control, not as a proposed method.'}</p>
      </Callout>
      {/* C4 mean_ap 0.19768750, boundary F 0.83440, count abs err 139.8, 22.1 ms and C7 mean_ap 0.1673125,
          boundary F 0.86284, count abs err 114.2, 9.6 ms: frontend/public/data/method-benchmark.json
          methods[3] and methods[6], .test and .compute. Learned leader N1 mean_ap 0.51859375, same file,
          current_bar.leader. C7 engine string and name: data-pipeline/fslab/model_registry.py C7 row. */}
