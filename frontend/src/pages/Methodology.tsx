...
export default function Methodology() {
  ...
  const thresholdTab = (
    <div className="prose">
      ...
      <p className="measure">{es
        ? 'C2 prueba la hipótesis opuesta: que el gradiente crudo ya contiene las cuencas correctas. Se calcula el gradiente morfológico por rango sobre un disco de radio 1, se toman los máximos locales del gradiente negado con distancia mínima 2 dentro del dominio de espuma, y se inunda el gradiente desde esos marcadores. Como todo mínimo regional del gradiente es una semilla, cada reflejo especular y cada hundimiento de textura se convierte en su propia cuenca. En la escena canónica polidispersa produce 1519 cuencas para 197 burbujas, con AP 0.005. En la prueba retenida su AP es 0.0173 con 71918 instancias predichas contra 17846 verdaderas y 5507 separaciones, y tarda 80.8 milisegundos por muestra, veinticuatro veces más que C1 por el volumen de etiquetas que arrastra.'
        : 'C2 tests the opposite hypothesis: that the raw gradient already contains the right basins. The morphological rank gradient is computed over a radius-1 disk, the local maxima of the negated gradient are taken with a minimum distance of 2 inside the froth domain, and the gradient is flooded from those markers. Because every regional minimum of the gradient is a seed, each specular highlight and every texture dip becomes its own basin. On the canonical polydisperse scene it produces 1519 basins for 197 bubbles at AP 0.005. On the held-out test its AP is 0.0173 with 71918 predicted instances against 17846 true ones and 5507 splits, and it takes 80.8 milliseconds per sample, twenty-four times C1, because of the label volume it carries.'}{' '}<Cite id="vincent1991" paren /></p>
