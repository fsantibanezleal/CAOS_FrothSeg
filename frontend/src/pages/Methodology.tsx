...
export default function Methodology() {
  ...
  const contractTab = (
    <div className="prose">
      ...
      {/* Title-level statement only. Crossref record for 10.1080/19392699.2026.2676005 read 2026-08-01,
          evidence in verification/froth-citations-2026-08-01.json. The paper's abstract is not retrievable,
          so nothing below its title is asserted here. */}
      <p className="measure">{es
        ? 'Esa regla no es una preferencia de este repositorio. El propio dominio la ha estudiado: un trabajo de 2026 en la literatura de flotación de carbón examina la fuga de datos inducida por intervalos de secuencia impropios y su impacto en la confiabilidad de modelos de aprendizaje profundo para reconocimiento de imágenes de espuma. El riesgo que nombra su título es exactamente el que la partición por grupo evita aquí, y por eso la partición está implementada en el código de datos y verificada por prueba, en lugar de quedar como una convención escrita.'
        : 'That rule is not a preference of this repository. The domain itself has studied it: a 2026 paper in the coal flotation literature examines improper sequence interval-induced data leakage and its impact on deep learning model reliability in froth image recognition. The risk its title names is exactly the one the grouped split avoids here, which is why the split is implemented in the dataset code and covered by a test rather than left as a written convention.'}{' '}<Cite id="zhang2026leakage" paren /></p>
      ...
      <Refs ids={['lin2014coco', 'weaire1999foams', 'aurenhammer1987', 'aldrich2010', 'zhang2026leakage']} label={refsLabel} />
    </div>
  );
  ...
  const thresholdTab = (
    <div className="prose">
      ...
      <p className="measure">{es
        ? 'C2 prueba la hipótesis opuesta: que el gradiente crudo ya contiene las cuencas correctas. Se calcula el gradiente morfológico por rango sobre un disco de radio 1, se toman los máximos locales del gradiente negado con distancia mínima 2 dentro del dominio de espuma, y se inunda el gradiente desde esos marcadores. No es una inundación desde todo mínimo regional, y conviene decirlo con precisión: la distancia mínima descarta los mínimos separados por menos de 2 píxeles de uno más fuerte, y la máscara de primer plano descarta los que caen fuera de la espuma. Aun así cada reflejo especular y cada hundimiento de textura que sobrevive a ese filtro se convierte en su propia cuenca. En la escena canónica polidispersa produce 1519 cuencas para 197 burbujas, con AP 0.005. En la prueba retenida su AP es 0.0173 con 71918 instancias predichas contra 17846 verdaderas y 5507 separaciones, y tarda 80.8 milisegundos por muestra, veinticuatro veces más que C1 por el volumen de etiquetas que arrastra.'
        : 'C2 tests the opposite hypothesis: that the raw gradient already contains the right basins. The morphological rank gradient is computed over a radius-1 disk, the local maxima of the negated gradient are taken with a minimum distance of 2 inside the froth domain, and the gradient is flooded from those markers. This is not a flood from every regional minimum, and the difference is worth stating exactly: the minimum distance drops minima lying within 2 pixels of a stronger one, and the foreground mask drops those outside the froth. Even so, every specular highlight and every texture dip that survives that filter becomes its own basin. On the canonical polydisperse scene it produces 1519 basins for 197 bubbles at AP 0.005. On the held-out test its AP is 0.0173 with 71918 predicted instances against 17846 true ones and 5507 splits, and it takes 80.8 milliseconds per sample, twenty-four times C1, because of the label volume it carries.'}{' '}<Cite id="vincent1991" paren /></p>
      {/* disk(1) rank gradient, min_distance=2: data-pipeline/fslab/science/segment.py lines 84-101 */}
      {/* 1519 basins for 197 bubbles, AP 0.005: data/derived/synth/poly-normal/benchmark.json (watershed_immersion) */}
      ...
      <Figure caption={es
        ...
        : 'An intensity profile across two touching bubbles, with the real constants of the renderer and of both methods. At t* the seam would cut; at 0.75 t* it does not, and the two caps stay one connected component. On the right, the basins flooded from the surviving seeds: peak_local_max on the negated gradient with a 2 px minimum separation inside the froth foreground, so closely spaced and out-of-mask minima are dropped.'}>
        <svg viewBox="0 0 760 300" className="fig-svg wide" role="img" aria-labelledby="thrTitle thrDesc">
          ...
          <text x="240" y="199" className="dg-box-sub">{es ? '76 predicciones para 197 verdaderas' : '76 predictions for 197 true'}</text>
          <text x="240" y="216" className="dg-box-sub">{es ? 'AP 0.137 · subsegmentación' : 'AP 0.137 · under-segmentation'}</text>
          <text x="240" y="240" className="dg-box-title">{es ? 'C2: toda cuenca del gradiente' : 'C2: every gradient basin'}</text>
          <text x="240" y="259" className="dg-box-sub">{es ? '1519 cuencas para 197 verdaderas' : '1519 basins for 197 true'}</text>
          <text x="240" y="276" className="dg-box-sub">AP 0.005</text>
  ...
  const watershedTab = (
    <div className="prose">
      ...
      <Equation
        ...
        caption={es
          ? 'E es la elevación inundada, M los marcadores, Ω el dominio de espuma. Para C4 y C5 la elevación es la transformada de distancia euclidiana exacta D negada: los centros de burbuja son sus valles. C3 inunda esa misma forma con otra elevación desde el 2026-08-01, E = -I, la imagen negada.'
          : 'E is the flooded elevation, M the markers, Ω the froth domain. For C4 and C5 the elevation is the negated exact Euclidean distance transform D: bubble centres are its valleys. C3 floods the same form with a different elevation since 2026-08-01, E = -I, the negated image.'}
      />

      <p className="measure">{es
        ? 'C4 es el piso genérico de los tres. Tras la adopción de la superficie publicada de C3 ya no es el más fuerte: C3 marca 0.2975 de AP retenida contra 0.1977 de C4. Dentro del dominio de espuma se calcula la transformada de distancia euclidiana EXACTA, no una aproximación de chamfer, y sus máximos locales con distancia mínima 4 píxeles se convierten en marcadores; luego se inunda la distancia negada con máscara. La distancia mínima de 4 píxeles es el radio de burbuja más pequeño resoluble y es una de las tres constantes que este carril hereda del tamaño del cuadro. Funciona porque para una celda convexa la distancia al borde tiene exactamente un máximo, así que un marcador por celda es lo correcto por geometría. En la prueba retenida obtiene AP 0.1977, AP50 0.4134, PQ 0.4022 y una distancia de Wasserstein de 2.590, la segunda del tier clásico detrás del 2.037 de C3, con 9188 instancias predichas y 1009 separaciones. Falla donde la hipótesis de convexidad se rompe: cuando dos celdas quedan fusionadas en la máscara de primer plano, la distancia tiene un solo máximo para las dos.'
        : 'C4 is the generic floor of the three. Since C3 adopted its published surface it is no longer the strongest: C3 records 0.2975 held-out AP against C4 0.1977. Inside the froth domain the EXACT Euclidean distance transform is computed, not a chamfer approximation, and its local maxima at a minimum distance of 4 pixels become the markers; the negated distance is then flooded under the mask. The 4-pixel minimum distance is the smallest resolvable bubble radius and is one of three constants this lane inherits from the frame size. It works because for a convex cell the distance to the border has exactly one maximum, so one marker per cell is right by geometry. On the held-out test it scores AP 0.1977, AP50 0.4134, PQ 0.4022 and a Wasserstein distance of 2.590, second in the classical tier behind C3 at 2.037, with 9188 predicted instances and 1009 splits. It fails where the convexity hypothesis breaks: when two cells stay fused in the foreground mask, the distance has a single maximum for both.'}</p>
      ...
      <p className="measure">{es
        ? 'C3 codifica el truco industrial canónico de la visión de espuma: cada burbuja lleva un reflejo bajo la iluminación de anillo, así que los reflejos son los centros. Se aplica la transformada de h-máximos con altura de contraste h = 0.12 sobre la imagen en gris, que suprime todo máximo regional más somero que h y deja solo los puntos brillantes salientes; esos se etiquetan como marcadores y, desde el 2026-08-01, se inunda la imagen negada en vez de la distancia negada. Si no sobrevive ningún reflejo limpio el método cae de vuelta a los marcadores de distancia de C4, una guarda honesta para cuadros sin brillo. Y aquí el banco es deliberadamente adverso: el generador jitterea el reflejo con desviación proporcional al radio y lo omite con probabilidad proporcional al mismo parámetro, precisamente para que un método sembrado en reflejos no pueda ganar de forma artificial.'
        : 'C3 encodes the canonical industrial trick of froth machine vision: every bubble carries a highlight under ring illumination, so the highlights are the centres. The h-maxima transform is applied with contrast height h = 0.12 on the grey image, suppressing every regional maximum shallower than h and leaving only the salient bright spots; those are labelled as markers and, since 2026-08-01, the negated image is flooded rather than the negated distance. If no clean highlight survives, the method falls back to the C4 distance markers, an honest guard for highlight-free frames. And here the benchmark is deliberately adversarial: the generator jitters the highlight with a standard deviation proportional to the radius and drops it with a probability proportional to the same parameter, precisely so a highlight-seeded method cannot win artificially.'}{' '}<Cite id="aldrich2010" paren /></p>
      {/* h=0.12 (adopted 2026-08-01, was 0.06) and the C4 fallback: data-pipeline/fslab/science/segment.py lines 47-56 */}

      <Equation
        tex={String.raw`\mathrm{HMAX}_h(f)=R_f^{\delta}\!\left(f-h\right),\qquad M=\operatorname{label}\bigl(\mathrm{HMAX}_{0.12}(I)\bigr)`}
        caption={es
          ? 'Transformada de h-máximos: R con superíndice δ es la reconstrucción morfológica por dilatación bajo la máscara f, y h la altura de contraste que un máximo debe superar para sobrevivir como marcador. Aquí h = 0.12 sobre la imagen en gris normalizada.'
          : 'H-maxima transform: R with superscript δ is morphological reconstruction by dilation under the mask f, and h the contrast height a maximum must exceed to survive as a marker. Here h = 0.12 on the normalised grey image.'}
      />

      <p className="measure">{es
        ? 'El resultado medido dice que el indicio de reflejo depende por completo de la superficie que se inunda. Hasta el 2026-08-01 C3 inundaba la transformada de distancia negada, que es la superficie de C4, y quedaba en AP 0.1031 y PQ 0.2490: la mitad de C4, con los mismos marcadores. Inundando la imagen negada, que es lo que publica su propia fuente, sube a AP 0.2196 y PQ 0.4409 con el mismo conjunto de marcadores y las mismas 29248 instancias predichas, y tras corregir despues la profundidad de inundacion llega a AP 0.2975 y PQ 0.5423 con 19544 instancias, y las fusiones bajan de 3009 a 1142. El desglose por condición muestra que el indicio sigue siendo frágil donde debe serlo: en la condición nominal polidispersa alcanza 0.379, y bajo la tormenta de brillo cae a 0.133, mientras C4 sostiene 0.089. En el carril de secuencias el mecanismo persiste, con 1296 eventos falsos en transporte nominal y 6208 bajo tormenta de brillo, porque un reflejo que aparece y desaparece nace y muere como identidad. C5 ataca el problema desde el otro lado: en vez de sembrar donde hay brillo, suprime cuencas someras. Se normaliza el mapa de distancia, se niega, y se aplica la transformada de h-mínimos con h = 0.08, que colapsa todo mínimo menos profundo que h antes de inundar. Conviene ser exacto con lo que h significa: como la distancia se divide por su propio máximo en cada imagen, h no es una profundidad en píxeles sino una FRACCIÓN del valor máximo de la transformada de distancia de ese cuadro, de modo que 0.08 suprime 1.6 píxeles en un cuadro cuyo máximo es 20 y otra profundidad en el cuadro siguiente. Es la única perilla del método y fija en la práctica la burbuja más pequeña resoluble, pero lo hace en escala relativa por imagen.'
        : 'The measured result says the highlight cue depends entirely on which surface it floods. Until 2026-08-01 C3 flooded the negated distance transform, which is C4\'s surface, and landed at AP 0.1031 and PQ 0.2490: half of C4, on the same markers. Flooding the negated image, which is what its own source publishes, takes it to AP 0.2196 and PQ 0.4409 on the identical marker set and the identical 29248 predicted instances, and correcting the flooding depth afterwards takes it to AP 0.2975 and PQ 0.5423 on 19544 instances, with merges falling from 3009 to 1142. The per-condition breakdown shows the cue is still fragile where it should be: on the nominal polydisperse condition it reaches 0.379, and under the glare storm it falls to 0.133, while C4 holds 0.089. In the sequence lane the mechanism persists, at 1296 false events on nominal transport and 6208 under the glare storm, because a highlight that appears and disappears is born and dies as an identity. C5 attacks the problem from the other side: instead of seeding where the glints are, it suppresses shallow basins. The distance map is normalised, negated, and the h-minima transform is applied with h = 0.08, collapsing every minimum shallower than h before flooding. It is worth being exact about what h means: because the distance is divided by its own maximum on every image, h is not a depth in pixels but a FRACTION of the largest distance-transform value in that frame, so 0.08 suppresses 1.6 pixels on a frame whose maximum is 20 and a different depth on the next frame. It is the only knob of the method and in practice it sets the smallest resolvable bubble, but it does so on a per-image relative scale.'}</p>
      {/* C3 held-out and per-condition AP: frontend/public/data/method-benchmark.json methods[2] */}
      {/* 1296 false events on poly-normal and 6208 on glare-storm, re-baked after the 2026-08-01 adoption:
          frontend/public/data/temporal/watershed_hmax.json sequences[poly-normal|glare-storm].event_false_positives */}
      ...
      <Figure caption={es
        ...
        <svg viewBox="0 0 760 322" className="fig-svg wide" role="img" aria-labelledby="wsTitle wsDesc">
          ...
          <desc id="wsDesc">{es
            ? 'La curva es la distancia negada al borde a lo largo de una línea que cruza tres burbujas. C3 siembra en los reflejos de la imagen en gris con h igual a 0.12, C4 en los máximos de distancia separados al menos 4 píxeles, y C5 suprime los mínimos más someros que 0.08 antes de inundar.'
            : 'The curve is the negated distance to the border along a line crossing three bubbles. C3 seeds at the grey-image highlights with h equal to 0.12, C4 at distance maxima separated by at least 4 pixels, and C5 suppresses minima shallower than 0.08 before flooding.'}</desc>
          ...
          <text x="60" y="220" className="dg-box-sub">{es ? 'C3 no usa esta curva: siembra en los h-máximos del gris con h = 0.12' : 'C3 does not use this curve: it seeds at grey h-maxima with h = 0.12'}</text>
  ...
  const regionTab = (
    <div className="prose">
      ...
      <p className="measure">{es
        ? 'C7 invierte el indicio y es el método específico del dominio. En vez de buscar los puntos brillantes busca las costuras oscuras. Un black top-hat con un disco de radio 3 aísla las estructuras oscuras más delgadas que el elemento estructurante, es decir exactamente las películas entre burbujas; la respuesta del top-hat se umbraliza por varianza entre clases para obtener la máscara de costuras, las costuras se quitan del primer plano y se eliminan los objetos de menos de 8 píxeles. Hasta el 2026-08-01 esas tapas encerradas ERAN las instancias; desde entonces son los marcadores de una watershed restringida (Meyer 1994) que inunda la propia respuesta del top-hat, así que cada tapa vuelve a crecer hasta la línea central de la costura en vez de quedarse corta. No hay ningún prior de reflejo. Es robusto a los brillos por construcción, porque un brillo es un máximo y el operador solo mira mínimos delgados.'
        : 'C7 reverses the cue and is the domain-specific method. Instead of looking for the bright spots it looks for the dark seams. A black top-hat with a radius-3 disk isolates dark structures thinner than the structuring element, which is exactly the films between bubbles; the top-hat response is thresholded by between-class variance to obtain the seam mask, the seams are removed from the foreground, and objects below 8 pixels are dropped. Until 2026-08-01 those enclosed caps WERE the instances; since then they are the markers of a constrained watershed (Meyer 1994) that floods the top-hat response itself, so every cap grows back to the seam centreline instead of stopping short of it. There is no highlight prior. It is robust to glints by construction, because a glint is a maximum and the operator only looks at thin minima.'}{' '}<Cite id="wang2003froth" paren /></p>
      ...
      <p className="measure">{es
        ? 'La razón por la que este indicio es el correcto es física, no heurística. Las películas de una espuma seca se encuentran de tres en tres a lo largo de un borde de Plateau, y el borde es una estructura oscura delgada que aparece en la imagen aunque el interior de la burbuja esté saturado. En el generador ese oscurecimiento es un término exponencial en la distancia euclidiana exacta al borde de celda, así que el valle es un rasgo geométrico genuino y no una textura añadida; en espuma real es la red de películas. El resultado medido acompaña, y desde el 2026-08-01 el motor hace lo que su nombre en el registro ya declaraba: en vez de restar la costura y etiquetar lo que queda, usa las tapas limpias como marcadores e inunda la respuesta del sombrero de copa negro, de modo que cada tapa vuelve a crecer hasta la línea central de la costura (Meyer 1994). En la prueba retenida C7 obtiene AP 0.2326, PQ 0.4382 y la mejor F de frontera del tier clásico, 0.8837, con 389 separaciones mientras predice 10809 instancias, frente a 744 separaciones sobre 6809 predicciones del watershed de h-mínima. Encabezó el tier clásico en AP hasta que se corrigió la profundidad de C3 el mismo día; ahora es segundo con 0.2326 detrás de C3 con 0.2975 y por delante de C4 con 0.1977, y en la escena canónica polidispersa registra AP 0.458 con 166 predicciones para 197 verdaderas, por delante de C4 con 0.401. El costo del cambio se declara donde se describe el cambio: el error relativo de d32 empeora, de 1.2584 a 1.4371, porque devolver cada tapa hasta la cresta agranda cada burbuja y C7 ya sobrestimaba el diámetro medio de Sauter.'
        : 'The reason this cue is the right one is physical, not heuristic. The films of a dry foam meet three at a time along a Plateau border, and the border is a thin dark structure that appears in the image even when the bubble interior is saturated. In the generator that darkening is an exponential term in the exact Euclidean distance to the cell edge, so the valley is a genuine geometric feature and not an added texture; in real froth it is the film network. The measured result follows, and since 2026-08-01 the engine does what its registry name already claimed: instead of subtracting the seam and labelling what is left, it uses the cleaned caps as markers and floods the black-top-hat response, so every cap grows back to the seam centreline (Meyer 1994). On the held-out test C7 scores AP 0.2326, PQ 0.4382 and the best boundary F of the classical tier, 0.8837, with 389 splits while predicting 10809 instances, against 744 splits over 6809 predictions for the h-minima watershed. It led the classical tier on AP until the C3 flooding depth was corrected the same day; it is now second at 0.2326 behind C3 at 0.2975 and ahead of C4 at 0.1977, and on the canonical polydisperse scene it records AP 0.458 with 166 predictions for 197 true bubbles, ahead of C4 at 0.401. The cost of the change is stated where the change is described: d32 relative error gets worse, 1.2584 to 1.4371, because growing every cap back to the ridge enlarges every bubble and C7 already over-estimated the Sauter mean diameter.'}{' '}<Cite id="weaire1999foams" paren /></p>
      {/* C7 held-out values: frontend/public/data/method-benchmark.json methods[6].test; canonical 166 preds,
          AP 0.458, C7 leads and C4 is second at 0.401: data/derived/synth/poly-normal/benchmark.json.
          d32 cost 1.2584 to 1.4371: verification/phase1-adoption.json changes[1].stated_cost */}
      ...
      <Figure caption={es
        ...
        <svg viewBox="0 0 760 300" className="fig-svg wide" role="img" aria-labelledby="regTitle regDesc">
          <title id="regTitle">{es ? 'Geometría de los dos indicios: brillo especular y valle de Plateau' : 'Geometry of the two cues: specular highlight and Plateau valley'}</title>
          <desc id="regDesc">{es
      ...
      <p className="measure">{es
        ? 'El límite de C7 está escrito en su propio operador, y la watershed restringida no lo levanta. El disco de radio 3 fija la anchura máxima de costura que puede aislar, así que un borde ancho de espuma acuosa lo rompe: bajo la condición acuosa cae a 0.020 mientras el watershed de distancia sostiene 0.126, y esa es su peor cifra de las 16 condiciones, seguida por el estresor compuesto de brillo con movimiento, 0.037. Si la costura no se detecta no hay marcador que inundar, así que hacer crecer las tapas no recupera una burbuja que nunca se separó: sigue prediciendo 10809 instancias frente a 17846 verdaderas, y su Wasserstein-1 se queda en 3.542 pese a la mejor F de frontera del tier. Lo que sí cambió el 2026-08-01 es el sesgo de tamaño, y en la dirección mala: devolver cada tapa hasta la cresta agranda cada burbuja y deja a C7 con el peor error relativo de d32 de los tres punteros, 1.4371 contra 1.2584 antes, frente al 1.1555 de C4 y al 0.1098 de C3. Bajo brillo, en cambio, es el clásico más fuerte con 0.149, y en espuma gruesa alcanza 0.527, la cifra más alta del tier clásico en esa condición y por delante del carril de SAM 2.1 sin prompts, que llega a 0.459 en la misma condición.'
        : 'The limit of C7 is written into its own operator, and the constrained watershed does not lift it. The radius-3 disk fixes the maximum seam width it can isolate, so a wide watery border breaks it: under the watery condition it falls to 0.020 while the distance watershed holds 0.126, and that is its worst figure of the 16 conditions, followed by the compound glare-with-motion stressor at 0.037. If the seam is never detected there is no marker to flood, so growing the caps back does not recover a bubble that was never separated: it still predicts 10809 instances against 17846 true ones, and its Wasserstein-1 stays at 3.542 despite the best boundary F of the tier. What did change on 2026-08-01 is the size bias, and in the wrong direction: growing every cap back to the ridge enlarges every bubble and leaves C7 with the worst d32 relative error of the three leaders, 1.4371 against 1.2584 before, next to C4\'s 1.1555 and C3\'s 0.1098. Under glare, by contrast, it is the strongest classical at 0.149, and on coarse froth it reaches 0.527, the highest figure of the classical tier on that condition and ahead of the unprompted SAM 2.1 lane, which reaches 0.459 on the same condition.'}</p>
      {/* C7 per-condition AP after the 2026-08-01 re-bake (watery 0.02025, glare-motion-compound 0.037,
          glare-storm 0.14875, coarse-froth 0.527): frontend/public/data/method-benchmark.json
          methods[6].test.robustness_by_condition; nPred 10809 against 17846 true and BSD W1 3.5419 in the
          same block; d32 1.43706 there and 1.25836 before, verification/phase1-adoption.json changes[1] */}
      ...
      <Callout variant="honest" title={es ? 'Lo que este par no autoriza a decir' : 'What this pair does not license'}>
        <p>{es
          ? 'La cabeza del tier clásico estuvo repartida hasta el 2026-08-01 y ya no lo está: tras corregir la profundidad de inundación de C3, ese método lidera TODOS los ejes registrados sobre las 64 muestras retenidas, con AP 0.2975, PQ 0.5423, F de frontera 0.9236, Wasserstein-1 de tamaños 2.037, error de conteo 64.9 y error relativo de d32 0.1098. C7 queda segundo en AP con 0.2326 y C4 tercero con 0.1977. Ni C7 ni C4 se movieron: solo se movió C3. C7 conserva el peor error de d32 de los tres punteros, 1.4371 frente al 1.1555 de C4, que es el costo declarado de devolver cada casquete hasta la cresta de la costura. Antes de esta corrección esta misma sección declaraba el reparto entre C7 y C4, y ese texto era cierto entonces y es falso ahora. Los tres siguen por debajo del líder aprendido, que marca 0.5186, así que el tier clásico no es una alternativa al carril entrenado sino su referencia. Dos de los tres cambios no se adoptaron por ganar un barrido: se adoptaron porque el motor no ejecutaba la fuente que el registro ya citaba. El tercero SI se adoptó por su puntaje. La profundidad de C3 se eligió como el argmax de la AP media de validación, y se publicó como una corrección de unidades; esa justificación era falsa y se retira, porque esa profundidad se aplica a la imagen de intensidad y nunca a la superficie inundada (CAOS_MANAGE plans/frothseg/research-2026-07-31/r2-correction-2026-08-02.md). Los tres se confirmaron sobre porciones de reserva que ningún barrido había observado, así que la medición se sostiene; lo que no se sostiene es que ninguno dependiera de un puntaje. Esto importa para leer la tabla: C3 es el único método clásico cuya constante residual fue reelegida, mientras C2 y C5 conservan valores por defecto con ganancias mayores sin reclamar en sus propios barridos, de modo que el tier no está afinado de manera uniforme. La procedencia de C7 también está declarada con cuidado: es una línea base de valle o cresta del dominio cuya cita exacta permanece bajo compuerta en el registro, de modo que se acredita a la tradición de delineación de espuma y no se presenta como la reproducción de un artículo concreto. Y su nombre en el registro dice lo que el motor hace: watershed restringido de valle de lamela. Hasta el 2026-07-31 el registro lo llamaba detector de costura oscura, porque el motor no inundaba nada; la adopcion del 2026-08-01 resolvio ese desajuste implementando el metodo, asi que el nombre y la cita de Meyer 1994 volvieron al registro. Sigue sin usar OpenCV. C6 se conserva como control negativo, no como método propuesto.'
          : 'The head of the classical tier was split until 2026-08-01 and is no longer: after C3\'s flooding depth was corrected, that method leads EVERY recorded axis over the 64 held-out samples, with AP 0.2975, PQ 0.5423, boundary F 0.9236, size-distribution Wasserstein-1 2.037, count error 64.9 and d32 relative error 0.1098. C7 is second on AP at 0.2326 and C4 third at 0.1977. Neither C7 nor C4 moved: only C3 did. C7 keeps the worst d32 of the three leaders, 1.4371 against C4\'s 1.1555, which is the stated cost of growing every cap back to the seam ridge. Before this correction this same section declared the split between C7 and C4, and that text was true then and is false now. All three remain below the learned leader at 0.5186, so the classical tier is not an alternative to the trained lane but its reference. Two of the three changes were not adopted for winning a sweep: they were adopted because the engine was not running the source the registry already cited. The third WAS adopted on its score. The C3 depth was selected as the argmax of validation mean AP, and it was published as a unit correction; that justification was false and is withdrawn, because this depth applies to the intensity image and never to the flooded surface (CAOS_MANAGE plans/frothseg/research-2026-07-31/r2-correction-2026-08-02.md). All three were confirmed on reserve slices no sweep had observed, so the measurement stands; what does not stand is the claim that none of them turned on a score. This matters for reading the table: C3 is the only classical method whose residual constant was re-selected, while C2 and C5 keep defaults with larger unclaimed gains on their own sweeps, so the tier is not uniformly tuned. The provenance of C7 is also stated carefully: it is a domain valley or ridge baseline whose exact citation remains gated in the registry, so it is credited to the froth-delineation tradition and is not presented as the reproduction of one specific paper. And its registry name says what the engine does: a lamella-valley constrained watershed. Until 2026-07-31 the registry called it a dark-seam detector, because the engine flooded nothing; the 2026-08-01 adoption resolved that mismatch by implementing the method, so the name and the Meyer 1994 provenance were restored. It still uses no OpenCV. C6 is kept as a negative control, not as a proposed method.'}</p>
      </Callout>
      {/* C3 mean_ap 0.2975 / PQ 0.5423 / boundary F 0.9236 / BSD W1 2.037 / count abs err 64.9 / d32 0.1098,
          C4 mean_ap 0.19768750 / boundary F 0.83440 / BSD W1 2.590 / count abs err 139.8, C7 mean_ap
          0.23262500 / boundary F 0.88369 / count abs err 114.2 / d32 1.43715:
          frontend/public/data/method-benchmark.json methods[2], methods[3] and methods[6], .test.
          Learned leader N1 mean_ap 0.51859375, same file, current_bar.leader. The first two adopted defaults
          and their reserve confirmation: verification/phase1-adoption.json. The third, C3_H_MAXIMA 0.06 to
          0.12, selected on validation and confirmed on reserve slice p4:
          verification/r2-c3-flooding-depth.json.
          C7 engine string and name: data-pipeline/fslab/model_registry.py C7 row. */}
  ...
  const objectTab = (
    <div className="prose">
      ...
      <p className="measure">{es
        ? 'Hay una limitación de plataforma que debe declararse porque afecta a la lectura del costo: la rueda oficial de TensorFlow 2.21 es solo CPU en Windows nativo, así que el repositorio fija la variante de CPU en Windows y la variante completa en Linux o WSL2. La corrida completada reporta CPU de forma explícita y su latencia media es 595.3 milisegundos por muestra; no debe describirse como GPU. El resultado queda por debajo de la barra y se conserva de todas formas, con el checkpoint oficial, la configuración, los umbrales, los registros de entrenamiento y la evidencia por caso, porque un resultado negativo implementado con procedencia verificable vale más que una fila ausente.'
        : 'There is a platform limitation that must be declared because it changes how the cost reads: the official TensorFlow 2.21 wheel is CPU-only on native Windows, so the repository pins the CPU variant on Windows and the full variant on Linux or WSL2. The completed run reports CPU explicitly and its mean latency is 595.3 milliseconds per sample; it must not be described as GPU. The result is below the bar and is kept anyway, with the official checkpoint, the configuration, the thresholds, the training logs and the per-case evidence, because an implemented negative result with verifiable provenance is worth more than a missing row.'}</p>
      {/* CPU-only TensorFlow on native Windows: docs/frameworks/10_stardist/README.md lines 12-15; 595.3 ms and hardware_lane cpu: frontend/public/data/method-benchmark.json methods[10].compute */}
      ...
      <Figure caption={es
        ...
        <svg viewBox="0 0 760 300" className="fig-svg wide" role="img" aria-labelledby="objTitle objDesc">
          ...
          <text x="8" y="270" className="dg-box-sub">L4: AP 0.1119 · PQ 0.3242 · {es ? 'F de frontera' : 'boundary F'} 0.7104 · 595.3 ms {es ? '(CPU)' : '(CPU)'}</text>
          <text x="8" y="290" className="dg-box-sub">L6: AP 0.2930 · PQ 0.5326 · W1 1.464 · 140.4 ms {es ? '(GPU)' : '(GPU)'}</text>
        </svg>
      </Figure>
  ...
  const promptTab = (
    <div className="prose">
      ...
      {/* Title-level statement only. Crossref record for 10.1016/j.ces.2025.121657 read 2026-08-01,
          evidence in verification/froth-citations-2026-08-01.json. ScienceDirect refuses the abstract and
          Crossref carries none, so no reported figure from that paper appears anywhere in this product. */}
      <p className="measure">{es
        ? 'La idea de aplicar un modelo fundacional de segmentación a la medición de tamaño en espuma no es exclusiva de este repositorio: existe un método publicado en 2025, basado en el Segment Anything Model, para la determinación precisa del tamaño de espuma en el proceso de flotación. Solo se cita su título, su revista y sus autores. Su resumen no es recuperable desde una fuente primaria, así que ninguna cifra suya se reproduce aquí y no se afirma ninguna comparación entre ese trabajo y esta fila.'
        : 'Applying a foundation segmentation model to froth size measurement is not unique to this repository: a 2025 paper publishes a Segment Anything Model-based method for precise froth size determination in the flotation process. Only its title, journal and authors are cited. Its abstract is not retrievable from a primary source, so none of its figures is reproduced here and no comparison between that work and this row is asserted.'}{' '}<Cite id="wang2025samfroth" paren /></p>

      <Refs ids={['kirillov2023', 'ravi2024sam2', 'chen2023slimsam', 'onnxruntimeweb', 'wang2025samfroth']} label={refsLabel} />
    </div>
  );
