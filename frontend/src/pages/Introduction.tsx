  ...
  return (
    <div className="page-body prose">
      ...
      <section>
        ...
        <Refs ids={['aldrich2010', 'wang2018', 'fu2019', 'jahedsaravani2017']} label={REFS} />
      </section>
      ...
      <section>
        ...
        <Refs ids={['wang2003froth', 'achanta2012slic', 'ronneberger2015unet', 'zhu2025gcfsegnet', 'fan2024parallel', 'schmidt2018stardist', 'redmon2016yolo', 'stringer2021cellpose', 'kirillov2023']} label={REFS} />
      </section>
      ...
      <section>
        ...
        <p className="measure">{es
          ? 'Lo que es ilustrativo, en cambio, es el dominio mismo. No hay imágenes reales de espuma en este repositorio, y no porque no se hayan buscado: al 2026-07-28 cada candidato público de espuma verificado contra la fuente primaria resultó sin licencia, no comercial o de pago, y no hay dataset de segmentación de espuma en Zenodo. La imagenología de espuma es dato operacional de planta y las plantas no lo publican. El único lane real aceptado es de un dominio adyacente, fotografías reales de objetos densos que se tocan, y está etiquetado como tal en el código, no solo en la prosa: la fuente lleva el campo de dominio como adyacente y el reporte de release particiona por ese campo, de modo que solo una fuente de espuma puede levantar el requisito de dato real. Sin eso, el error de release sigue diciendo que no hay fuente real de espuma con licencia y calibración aceptada, y seguirá diciéndolo por mucha evidencia adyacente que se acumule.'
          : 'What is illustrative, by contrast, is the domain itself. There are no real froth images in this repository, and not for want of looking: as of 2026-07-28 every public froth candidate verified against its primary source turned out to be unlicensed, non-commercial or paywalled, and Zenodo returns no froth segmentation dataset. Froth imagery is operational plant data and plants do not publish it. The only accepted real lane is from an adjacent domain, real photographs of dense touching objects, and it is labelled as such in code and not only in prose: the source carries its domain field as adjacent and the release report partitions by that field, so only a froth source can lift the real-data requirement. Failing that, the release error still reads that no licensed, calibrated real froth held-out source has been accepted, and it will keep reading that no matter how much adjacent evidence accumulates.'}</p>

        <p className="measure">{es
          ? 'Ese lane adyacente ya entregó el resultado más incómodo del repositorio, y está publicado en lugar de omitido. Corrido sin cambios, con los ajustes calibrados en espuma sintética intactos, sobre 64 imágenes reales retenidas, el ranking sintético no se transfiere: el líder interno del banco de espuma cae de AP 0.519 a 0.125, los seis modelos entrenados en el repositorio se degradan sin excepción (delta medio -0.243) y seis de los siete métodos clásicos mejoran, con un delta medio clásico de +0.088. El séptimo es el contraejemplo y conviene nombrarlo: el watershed por inmersión sin marcadores ya estaba en 0.017 sobre espuma y entrega 0.0 sobre las imágenes reales, un delta de -0.017, así que el nivel clásico gana como nivel mientras su miembro más débil no tiene nada que ganar. La lectura honesta es angosta y conviene decirla exacta: el ranking de espuma es en buena parte una propiedad del generador, y nada aquí demuestra todavía que algún método sea bueno en espuma real. Tampoco muestra lo contrario, que el modelo fundacional sea mejor en espuma: ese dataset es su dominio de preentrenamiento y juega de local, mientras los especialistas reciben otra modalidad con sus umbrales de espuma puestos. Lo que la prueba mide es robustez al cambio de dominio.'
          : 'That adjacent lane has already produced the most uncomfortable result in the repository, and it is published rather than omitted. Run unchanged, with the settings calibrated on synthetic froth left intact, over 64 real held-out images, the synthetic ranking does not transfer: the internal froth-benchmark leader falls from AP 0.519 to 0.125, all six models trained in the repository degrade without exception (mean delta -0.243) and six of the seven classical methods improve, at a classical mean delta of +0.088. The seventh is the counterexample and it is worth naming: the marker-less immersion watershed was already at 0.017 on froth and returns 0.0 on the real images, a delta of -0.017, so the classical tier gains as a tier while its weakest member has nothing to gain. The honest reading is narrow and worth stating exactly: the froth ranking is substantially a property of the generator, and nothing here yet demonstrates that any method is good at real froth. Nor does it show the converse, that the foundation model is better on froth: that dataset is its pretraining domain and it is playing at home, while the specialists are handed a different modality with their froth thresholds intact. What the test measures is robustness to domain shift.'}{' '}<Cite id="stringer2021cellpose" paren /></p>
        ...
        {/* Sources for the transfer figures above, all in this repository:
            ...
            classical mean delta +0.088 over 7 methods, in-repo trained mean delta -0.243 over 6) and
