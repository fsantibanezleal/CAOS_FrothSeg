...
function Cases({ es, benchmark }: { es: boolean; benchmark: SamBenchmarkDoc | null }) {
  ...
  return (
    <div className="prose">
      ...
      <p className="measure">
        {es
          ? 'La evidencia puntuada más antigua sobre estos trece casos es un estudio de sensibilidad: preguntar si una grilla regular de puntos recupera instancias sin ningún ajuste específico a la tarea. Se conserva porque responde una pregunta que la matriz retenida no hace, y se conserva separada porque su protocolo no es el de la comparación principal. Las barras muestran el AP por caso contra la referencia clásica del mismo caso; el verde indica que la grilla de prompts iguala o supera esa referencia. Esa referencia es el mejor método clásico de cada caso, así que la adopción del 2026-08-01 la movió sin tocar una sola predicción de la grilla: el AP medio de la referencia subió de 0.262 a 0.351 con las dos primeras adopciones y a 0.402 al corregirse la profundidad de C3, la ventaja media de la grilla pasó de +0.103 a +0.014 y luego a -0.037, y los casos ganados cayeron de 10 a 5 y después a 4 sobre 12. La grilla ahora pierde contra la referencia clásica. El estudio se conserva con su nueva lectura en vez de con la anterior.'
          : 'The oldest scored evidence over these thirteen cases is a sensitivity study: asking whether a regular point grid recovers instances with no task-specific tuning at all. It is retained because it answers a question the held-out matrix does not ask, and it is kept separate because its protocol is not that of the primary comparison. The bars show per-case AP against the same case’s classical reference; green marks where the prompt grid matches or exceeds that reference. That reference is the best classical method on each case, so the 2026-08-01 adoption moved it without touching a single grid prediction: the mean reference AP rose from 0.262 to 0.351 with the first two adoptions and to 0.402 when C3’s flooding depth was corrected, the grid’s mean advantage went from +0.103 to +0.014 and then to -0.037, and cases won fell from 10 to 5 and then to 4 out of 12. The grid now loses to the classical reference. The study is kept with its new reading rather than its old one.'}
      </p>
